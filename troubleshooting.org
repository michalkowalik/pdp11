...
- [ ] how often is RTI in user context called?
- [X] test if RTI manages the mode switch and restore correctly
  - [X] user -> kernel -> user
  - [X] kernel -> kernel -> kernel
- [ ] Do I switch between user and kernel in the MMU correctly? <- add test!
- [ ] shouldn't be the previous mode set to Kernel after returning from the interrupt?
